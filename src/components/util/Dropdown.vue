...
<script>
import ConnectedOverlayScrollHandler from '../../util/ConnectedOverlayScrollHandler'
  ...
  methods: {
    ...
    alignOverlay() {
      if (this.appendTo) {
        ...
        this.$refs.overlay.style.minWidth =
          DomHandler.getOuterWidth(this.$refs.container) + 'px'
      } else {
    ...
    searchOptionInRange(start, end) {
      for (let i = start; i < end; i++) {
        ...
        let label = this.getOptionLabel(opt).toLocaleLowerCase(
          this.filterLocale,
        )
        if (
          label.startsWith(
        ...
        return this.options.filter(
          option =>
            this.getOptionLabel(option)
              ...
              .indexOf(this.filterValue.toLocaleLowerCase(this.filterLocale)) >
            -1,
        )
