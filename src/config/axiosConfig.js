...
const checkSession = function() { // TODO] currentSession 호출 없이 accessToken 만료(현재 1시간)되면 자동 refresh가 안되는 현상? 테스트 필요. 1시간에 한번씩 수동 refresh 돌려야할지도?
  axios.interceptors.request.use(function(config) {
    return new Promise((resolve) => {
      Auth.currentSession() // INFO] currentSession vs currentAuthenicatedUser: https://stackoverflow.com/questions/55739848/what-is-the-difference-between-auth-currentauthenticateduser-and-auth-currents
        .then((session) => {
